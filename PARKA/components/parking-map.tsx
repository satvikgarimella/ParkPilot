...
export function ParkingMap({ onFavoriteToggle }: ParkingMapProps) {
  ...
  const fetchParkingSpots = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/parking?lat=${userLocation.lat}&lng=${userLocation.lng}&radius=0.02`,
      )
      if (response.ok) {
